...
maps = L.map('svg-map').setView([-15.793889, -47.882778], 12,{ zoomControl: false });
mapLink = '<a href="http://openstreetmap.org">OpenStreetMap</a>';
L.tileLayer('https://{s}.tile.openstreetmap.org/{z}/{x}/{y}.png', {
    attribution: '&copy; ' + mapLink + ' Contributors', maxZoom: 18, minZoom:12,
}).addTo(maps);
maps._initPathRoot()
...
function lerTodosNos() {
    ...
    var elementos = svg.selectAll("g.nodes").data(mapdata.allNodes, function (d, i) { return d.name; });

    var inseriorNode = elementos.enter().append("g")
        .attr("class", "nodes");
    ...
    inseriorNode.append("circle")
        ...
        .on('contextmenu', function (d, i) { inicioFinalCaminho(i); })
        .call(drawManager)
...
function lertodasLinhas() {
    ...
    var elementos = svg
        .selectAll("g.line")
        .data(mapdata.caminhos, function (d) { return d.id });

    var newelementos = elementos.enter();


    var group = newelementos
        .append("g")
        .attr("class", "line");
